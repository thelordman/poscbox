...
import me.lord.poscbox.utilities.DiscordUtil;
		...
		if (!event.isWebhookMessage()) {
			...
			if (!(member.getUser() == Discord.jda.getSelfUser() && (message.getContentDisplay().contains(":") || !message.getEmbeds().isEmpty()))) {
				if (event.getChannel() == Discord.MINECRAFT_CHAT || event.getChannel() == Discord.STAFF_CHAT || event.getChannel() == Discord.ADMIN_CHAT) {
					...
					if (event.getChannel() == Discord.STAFF_CHAT) Bukkit.broadcast(finalMessage, "poscbox.command.staffchat");
					else if (event.getChannel() == Discord.ADMIN_CHAT) Bukkit.broadcast(finalMessage, "poscbox.command.adminchat");
